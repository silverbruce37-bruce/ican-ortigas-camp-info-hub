export interface WaymakerNode {
    ...
    x: number; // 3D Coordinate X (-50 to 50)
    y: number; // 3D Coordinate Y (-50 to 50)
    z: number; // 3D Coordinate Z (-50 to 50)
}

export const waymakerNodes: WaymakerNode[] = [
    {
...
export const allTags = Array.from(new Set(waymakerNodes.flatMap(node => node.tags)));
